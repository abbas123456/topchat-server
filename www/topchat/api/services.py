...
class CurlService(object):
    ...
    def get_response(self, relative_url):
        if self.settings['APPLICATION_SCHEME'] == 'http':
            return self.get_http_response(relative_url)
        else:
            return self.get_https_response(relative_url)
    ...
    def post(self, relative_url, postfields):
        if self.settings['APPLICATION_SCHEME'] == 'http':
            self.http_post(relative_url, postfields)
        else:
            self.https_post(relative_url, postfields)
    ...
    def delete(self, relative_url):
        if self.settings['APPLICATION_SCHEME'] == 'http':
            self.http_delete(relative_url)
        else:
            self.https_delete(relative_url)
    ...
    def https_delete(self, relative_url):    
        curl = pycurl.Curl()
        curl.setopt(curl.URL, "{0}{1}".format(self.settings['APPLICATION_URL_HOSTNAME'], relative_url))
        curl.setopt(pycurl.POSTFIELDS, "_method=DELETE")
        curl.setopt(pycurl.SSL_VERIFYPEER, self.settings['SSL_VERIFY_PEER'])
...
class ApiService(object):
    ...
    def get_all_rooms(self):
        ...
        rooms = json.loads(self.curl_service.get_response(relative_url))
        return rooms or {}

    def get_room_by_room_number(self, room_number):
        relative_url = "rooms/{0}/.json".format(room_number)
        room = json.loads(self.curl_service.get_response(relative_url))
    ...
    def ban_user_from_room(self, user):
        ...
        self.curl_service.post(relative_url, "banned_user={0}&room={1}".format(user.id, user.room.id))